...
class cSTorM32RcCmd():
    ...
    def finalize(self,noresponse=None):
        if noresponse == None:
            ...
            print("nonresponse is true")
        else:
            self.datastream = b'\xFA'
            print("nonresponse is false")
        if self.len == b'\x00':
            # self.datastream += self.len + self.cmd + self.crc    
            # self.datastream+=self.len+b'\x02'+b'\x33\x34'
            message = "\xf9\x00\x02\x33\x34"

            self.datastream=message.encode()
        else:
