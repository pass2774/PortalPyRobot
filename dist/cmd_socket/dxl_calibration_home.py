...
import time
import json
from src.dxl_registerMap import *
...
def get_calib_pos():
  for idx, description in calib_guide.items():
    print("Quick Calibration:: Set the robot to position ",idx, "- "+description)
...
def setCalibMap():
  ...
  idx=2
  range_map["raw"][idx]=[calib_pos_dxl[0][idx]+3541,calib_pos_dxl[0][idx]]
  idx=3
  range_map["raw"][idx]=[calib_pos_dxl[0][idx]-1034,calib_pos_dxl[0][idx]+1034]
  idx=4
  range_map["raw"][idx]=[calib_pos_dxl[0][idx]+1023,calib_pos_dxl[0][idx]-1023]
  idx=5
  range_map["raw"][idx]=[calib_pos_dxl[0][idx]-1023,calib_pos_dxl[0][idx]+1023]
  idx=6
  range_map["raw"][idx]=[calib_pos_dxl[0][idx]-800,calib_pos_dxl[0][idx]]
